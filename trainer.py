...
class Trainer:
    ...
    def __call__(self):
        ...
        self.optimizer = optim.SGD(self.net.parameters(), lr=self.args.lr, momentum=0.9, weight_decay=self.args.wd)
    ...
    def train(self, epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            reg_loss_conv = torch.tensor([0.0], requires_grad=True).to(device=train_loss.device)
            if self.args.r:
                for (kernel, input_size) in self.net.module.get_all_kernels():
                    bound = bounds.estimate(kernel, n=input_size[0], name_func=self.args.bound, n_iter=self.args.bound_n_iter)
                    reg_loss_conv = reg_loss_conv + F.threshold(bound, self.args.threshold_reg, 0.0)
